...
# Run tests with: make test PYTHON=$(VENV_PYTHON)  (or activate the venv first)
venv:
	uv venv --python $(PYTHON) --python-preference only-system --system-site-packages $(VENV)
...
venv-gpu:
	uv venv --python python3.12 --python-preference only-system --system-site-packages $(VENV)

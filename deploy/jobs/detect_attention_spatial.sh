#!/usr/bin/env bash
# Detection: attention-only spatial-contrast (LOCOS minus OV projection)
set -euo pipefail
...
echo "=== Attention-spatial detection: ${MODEL} (${DATASET}) ==="

python locos/detectors/attention_spatial.py \
    --model "${MODEL}" \
    --dataset "${DATASET}" \

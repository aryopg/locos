...
# ---------------------------------------------------------------------------
# NLI-based factuality scorers
# ---------------------------------------------------------------------------


def factkb_score(prediction: str, source: str) -> float:
    """Compute FactKB factuality score for a single prediction/source pair.

    Uses the ``bunsenfeng/FactKB`` RoBERTa model (NLI-style binary
    classification).  Input is ``[prediction, source]`` — the model returns
    a probability that the prediction is factually consistent with the source.

    Requires ``transformers`` and ``torch``.

    Returns:
        Float in [0, 1].  Higher means more factual.
    """
...
def factkb_score_batch(predictions: list[str], sources: list[str], batch_size: int = 16) -> list[float]:
    """Compute FactKB factuality scores for a batch of prediction/source pairs.

    Loads the model once and scores all pairs.  Processes in mini-batches
    of *batch_size* to manage GPU memory.

    Returns:
        List of floats in [0, 1], one per pair.
    """
    assert len(predictions) == len(sources), "predictions and sources must have the same length"
    import torch  # lazy import
    from transformers import AutoModelForSequenceClassification, AutoTokenizer
    ...
    for start in range(0, len(predictions), batch_size):
        ...
        pairs = [[p, s] for p, s in zip(batch_preds, batch_srcs)]
